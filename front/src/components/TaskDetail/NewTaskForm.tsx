      ...
      <div className="space-y-4 overflow-y-auto flex-1 pb-4">
        <div>
          ...
          <div className="flex items-center gap-2">
            {!isRootTask && nextNumbering && (
              <span className="text-dark-text-secondary text-sm flex-shrink-0">{nextNumbering}</span>
            )}
            <input
              type="text"
              value={title}
        ...
        <div>
          <label className="block text-sm font-bold mb-1 light:text-slate-700">负责智能体</label>
          <select
            value={assigneeId}
            onChange={(e) => setAssigneeId(e.target.value)}
            className="w-full px-3 py-2 border border-dark-border rounded focus:outline-none focus:ring-2 focus:ring-brand-blue text-sm text-dark-text-primary light:bg-white light:text-slate-900"
